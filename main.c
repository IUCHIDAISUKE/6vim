...
void editorUpdateRow(erow *row)
{
    ...
    free(row->render);
    row->render = malloc(row->size + tabs * (ROKUVIM_TAB_STOP - 1) + 1);
...
void editorDrawRows(struct abuf *ab)
{
    ...
    for (y = 0; y < E.screenrows; y++)
    {
        ...
        if (filerow >= E.numrows)
        {
            ...
            {
                char welcome[80];
                int welcomelen = snprintf(welcome, sizeof(welcome), "6vim editor -- version %s", ROKUVIM_VERSION);
